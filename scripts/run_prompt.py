...
# Client: 180s timeout; disable internal retries so we control them.
client = OpenAI(
...
def _extract_json(text: str) -> list[dict]:
    """Best-effort JSON extractor: direct parse, then fenced block, then first array."""
    try:
...
def _call_responses_once(system_msg: str, user_prompt: str):
...
def _call_responses_with_retries(system_msg: str, user_prompt: str, attempts: int = 3, base_delay: float = 1.5):
    """
    Try up to `attempts` times with exponential backoff on network/timeout errors.
    """
    for i in range(1, attempts + 1):
        try:
            return _call_responses_once(system_msg, user_prompt)
        except (httpx.ReadTimeout, httpx.ConnectTimeout, httpx.HTTPError, APITimeoutError) as e:
            if i == attempts:
                print(f"[error] OpenAI request failed after {attempts} attempts: {type(e).__name__}: {e}", flush=True)
                raise
            delay = base_delay * (2 ** (i - 1))
            print(f"[warn] Attempt {i} failed ({type(e).__name__}: {e}); retrying in {delay:.1f}s…", flush=True)
            time.sleep(delay)
...
def call_llm(prompt: str) -> list[dict]:
    ...
    """
    system_msg = (
        "You must use web search before answering. Find 8 recent news articles "
    ...
    try:
        resp = _call_responses_with_retries(system_msg, prompt, attempts=3, base_delay=1.5)
    except Exception:
        sys.exit(1)
